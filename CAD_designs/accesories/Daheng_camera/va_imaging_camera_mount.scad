...
// Plate Module
module plate(width = 70, length = 80, thickness = 4, hole_dia = 3, hole_margin = 5) {
    difference() {
        cube([width, length, thickness]);
        
        translate([hole_margin, hole_margin, -1])
            cylinder(h = thickness + 2, d = hole_dia, $fn = 50);
        translate([width - hole_margin, hole_margin, -1])
            cylinder(h = thickness + 2, d = hole_dia, $fn = 50);
        translate([hole_margin, length - hole_margin, -1])
            cylinder(h = thickness + 2, d = hole_dia, $fn = 50);
        translate([width - hole_margin, length - hole_margin, -1])
            cylinder(h = thickness + 2, d = hole_dia, $fn = 50);
    }
}
...
// Backplate Module
module back_plate(thickness = 4, height = 79, width = 80, hole_dia = 5.4, edge_margin = 10) {
    difference() {
        cube([thickness, width, height]);
        
        for (y = [edge_margin, width / 2, width - edge_margin]) {
            for (z = [edge_margin, (height / 2) - 10, height - edge_margin]) {
                translate([-1, y, z])
                    rotate([0, 90, 0])
                        cylinder(h = thickness + 2, d = hole_dia, $fn = 50);
            }
...
// Brace Module
module triangle_brace(thickness = 4, depth = 20, height = 30) {
...
translate(brace1_pos)
    rotate([90, 0, 0])
        triangle_brace();

translate(brace2_pos)
    rotate([90, 0, 0])
        triangle_brace();
